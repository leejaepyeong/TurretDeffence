    ...
    public void PreLoad()
    {
        ...
        for (int cnt = 0; cnt < this._PoolOptions.Count; cnt++)
        {
            ...
            for (int i = poolOptions.totalCount; i < poolOptions.preLoad; i++)
            {
                ...
                bool isUI = LayerMask.LayerToName(trans.gameObject.layer) == "UI" ? false : true;
                
                //parent the new instance to this transform
                trans.SetParent(container, isUI);
            }
        }
    }
    ...
    public GameObject Spawn(GameObject prefab, Vector3 position, Quaternion rotation)
    {
        ...
        for (int cnt = 0; cnt < this._PoolOptions.Count; cnt++)
        {
            ...
            if (poolOptions.prefab == prefab)
            {
                ...
                {
                    isUI = LayerMask.LayerToName(obj.layer) == "UI" ? false : true;
                    trans.SetParent(container, isUI);
                }
        ...
        isUI = LayerMask.LayerToName(newObj.layer) == "UI" ? false : true;
        //parent it to this transform
        newObj.transform.SetParent(container, isUI);
        //add returned instance to the list of all instances
